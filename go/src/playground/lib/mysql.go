// Functions for opening and configuring a connection to a MySQL-like database,
// with optional TLS support.
//
// Functions in this file are not thread-safe. However, the returned *sql.DB is.
// TODO(ivanpi): Duplicated in identityd sql.go. Move to shared location.
...
func NewDBConn(sqlConfig *SqlConfig, txIsolation string) (*sql.DB, error) {
	return openDBConn(configureDBConn(sqlConfig.wrapped, txIsolation))
}

func configureDBConn(sqlConfig *iSqlConfig, txIsolation string) string {
	params := url.Values{}
...
func openDBConn(dataSrcName string) (*sql.DB, error) {
...
// registerTLSConfig sets up the SQL connection to use TLS encryption.
// For more information see https://github.com/go-sql-driver/mysql/#tls
func registerTLSConfig(config *iSqlConfig) error {
